...
function Dashboard() {
  ...
  const favoritesGrid = favorites.length ? (
    <div className="space-y-3">
      <h2 className="text-base font-semibold text-white/85">รายการที่บันทึกไว้</h2>
        ...
        {favorites.map((item) => (
          ...
          >
            <div className="aspect-[4/3] w-full overflow-hidden">
              <img className="h-full w-full object-cover" src={item.cover} alt={item.title} />
            </div>
            <p className="px-3 pb-3 pt-2 text-sm font-medium text-white/80">{item.title}</p>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="relative z-10 flex w-full max-w-5xl flex-col gap-10 text-white">
        <header className="flex flex-col gap-4 text-center md:text-left">
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              <h1 className="text-3xl font-semibold md:text-4xl">พื้นที่จัดการข้อมูลของคุณ</h1>
              <p className="text-sm text-white/70 md:text-base">
                เพิ่มโพสต์ใหม่ บันทึกรายการที่ชอบ และดูรายละเอียดเพิ่มเติมภายในอินเทอร์เฟซเดียว
              </p>
            </div>
            <button
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="flex flex-col gap-2 px-4 py-4 text-white/90">
                          ...
                          <p className="text-sm leading-relaxed text-white/70">{post.body}</p>
                        </div>
                      </button>
                    </div>
                  );
              ...
              {selectedPost ? (
                <div className="grid gap-6 rounded-2xl border border-white/10 bg-white/5 p-6 text-white/90 backdrop-blur-md md:grid-cols-[280px,1fr]">
                  <div className="overflow-hidden rounded-2xl border border-white/10">
                    <img className="h-full w-full object-cover" src={homeImage} alt={selectedPost.title} />
                  </div>
                  <div className="flex flex-col gap-3">
                    <h3 className="text-2xl font-semibold text-white">{selectedPost.title}</h3>
                    <p className="text-sm leading-relaxed text-white/75">{selectedPost.body}</p>
                  </div>
                </div>
              ) : null}
